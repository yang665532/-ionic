...
export class CourseDetailPage implements OnInit {
  ...
  ngOnInit() {
      ...
      this.segValue = this.navParams.get('segValue');
      console.log(this.id.substring(0, 7));

      if(this.id.substring(0, 7) !== 'youtube' && this.segValue == "part"){
        this.related = true;
      ...
      }else if(this.id.substring(0, 7) !== 'youtube' && this.segValue == "type"){
        this.related = true;
        this.dataService.getTypeDesc(this.id).subscribe(data => {
          this.acups = data.map(e => {
            return {
              acup_pic: e.payload.doc.data()['acup_pic'],
              desc: e.payload.doc.data()['detail']
